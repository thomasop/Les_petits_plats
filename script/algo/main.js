    ...
    mainInput.addEventListener("input", function() {
        ...
        if (mainInput.value.length > 2 && regex.test(mainInput.value)) {
            ...
            for(let i = 0; i < recipes.length; i++) {
                if (recipes[i].name.toLowerCase().includes(mainInput.value) == true || recipes[i].description.toLowerCase().includes(mainInput.value) == true) {
                    getRecipe.push(recipes[i])
                    continue;
                }

                for (let y = 0; y < recipes[i].ingredients.length; y++) {
                    if (recipes[i].ingredients[y].ingredient.toLowerCase().includes(mainInput.value) == true) {
                        getRecipe.push(recipes[i])
                        continue;
                    }
                }
            ...
            var message = document.querySelector(".message");
            message.style.display = "block";
            message.textContent = "3 caratères minimum pour effectué une recherche, les chiffres et caractères spéciaux ne sont pas autorisé.";
            if (tag.childNodes.length > 0) {
                data.data = initialArrayOfData
                checkWhichTagPresent(data)
            } else {
