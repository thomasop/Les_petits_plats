...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.mainInput').addEventListener('focus', function() {
        if (ingredient.style.display == "block") {
            ...
            ingredient.previousSibling.previousSibling.previousSibling.style.display = "block";
            ...
            appareil.previousSibling.previousSibling.previousSibling.style.display = "block";
        } else if (ustensil.style.display == "block") {
            ustensil.style.display = "none";
            ustensil.previousSibling.previousSibling.style.display = "block";
            ustensil.previousSibling.previousSibling.previousSibling.style.display = "block";
        }
    });
...
function closeInput(input1, input2) {
    if (input1.style.display == "block") {
        ...
        input1.previousSibling.previousSibling.previousSibling.style.display = "block";
    }
    if (input2.style.display == "block") {
        input2.style.display = "none";
        input2.previousSibling.previousSibling.style.display = "block";
        input2.previousSibling.previousSibling.previousSibling.style.display = "block";
    }
}
...
function checkWhichTagPresent(recipes, type) {
    ...
    if (data.data.length > 0) {
        if (data.ingredient) {
            updateDataWithTag(recipes, type, getTagIngredientData, getTagApplianceData, getTagUstensilData, data.ingredient, data.appliance, data.ustensil)
            displayData(data.data)
        } else if (data.appliance) {
            updateDataWithTag(recipes, type, getTagApplianceData, getTagIngredientData, getTagUstensilData, data.appliance, data.ingredient, data.ustensil)
            displayData(data.data)
        } else if (data.ustensil) {
            updateDataWithTag(recipes, type, getTagUstensilData, getTagApplianceData, getTagIngredientData, data.ustensil, data.appliance, data.ingredient)
            displayData(data.data)
        }
        else {
            displayData(data.data)
...
function updateDataWithTag(recipes, type, firstFunctionTag, secondFunctionTag, thirdFunctionTag, firstVarTag, secondVarTag, thirdVarTag) {
    if (secondVarTag && secondVarTag.length > 0 && type == undefined) {
        secondFunctionTag(data.data);
    } 
    if (thirdVarTag && thirdVarTag.length > 0 && type == undefined) {
        thirdFunctionTag(data.data);
    } 
    if (type == 'deleteTag') {
        data.data = initialArrayOfData
        checkMainInputWithTag()
        if (secondVarTag && secondVarTag.length > 0 || thirdVarTag && thirdVarTag.length > 0 || firstVarTag && firstVarTag.length > 0) {
            ...
            if (firstVarTag && firstVarTag.length > 0) {
                firstFunctionTag(data.data);
            }
        } else {
            firstFunctionTag(data.data);
        }
    } else {
       firstFunctionTag(recipes.data);
    }
}

// Get data of json with ingredient tag
function getTagIngredientData(dataTag) {
    data.ingredient.forEach(function(element, index, array) {
        var getRecipe = dataTag.filter((recipe) => {
...
function getTagApplianceData(dataTag) {
...
function getTagUstensilData(dataTag) {
